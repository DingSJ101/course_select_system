...
def load_student_from_file(filepath):
    ...
    if filetype == 'txt':
        ...
        maxNum = Student.query.order_by(Student.StudentNum).first().StudentNum
        for i , line in enumerate(f.readlines()):
            order , name , dept , major  =  line.split('\t')
            deptnum = deptname2deptnum(dept)
            majornum = majorname2majornum(major)
            new_student = Student(str(int(maxNum)+i),name,deptnum,majornum)
            try:
    ...
    if filetype == 'excel':
        ...
        for i in range(nrows):
            order , name , dept , major  =  table.row_values(i)
            deptnum = deptname2deptnum(dept)
            majornum = majorname2majornum(major)
            new_student = Student(str(int(maxNum)+i),name,deptnum,majornum)
            try:
